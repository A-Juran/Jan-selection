<template>
  <div>
    <el-card class="box-card">
      ...
      <el-table border :data="records" style="margin: 0.625rem 0">
        ...
        <el-table-column prop="" label="品牌操作" fixed="right">
          <template #default="{ row, $index }">
            <el-button size="small" link @click="$event => updateTrademark(row)">Edit</el-button>
            <el-popconfirm title="确认删除?" @confirm="$event => delTrademark(row)">
              <template #reference>
      ...
      <div class="demo-pagination-block">
        <el-pagination v-model:current-page="pageNo" v-model:page-size="limit" :page-sizes="[10, 15, 20]" background
    ...
    <!-- 
     ...
     -->
    <el-dialog v-model="dialogTableVisible" :title="trademarkInfoParams.id ? '修改品牌' : '添加品牌'" width="25%">
      <el-form label-position="left" :model="trademarkInfoParams" :rules="formRules" ref="ruleFormRef">
        <el-form-item prop="tmName">
          <el-input placeholder="品牌名称" autocomplete="off" v-model="trademarkInfoParams.tmName" />
        </el-form-item>
        <el-form-item class="preview_logo" label="(默认logo👉(上传可见自己logo、点击商品即展示商品logo))">
          <!--  v-if="imgPreviewSwitch" -->
          <el-image style="width: 100px; height: 100px;" :src="trademarkInfoParams.logoUrl" fit="cover" />
        </el-form-item>
        <el-form-item label-width="0" prop="logoUrl">
          <el-upload class="upload-demo" v-model="trademarkInfoParams.logoUrl" drag action="/api/admin/product/fileUpload"
            :before-upload="beforeAvatarUpload" :on-success="handleUploadFileSuccess" multiple>
            <el-icon class="el-icon--upload">
              <upload-filled />
            </el-icon>
            <div class="el-upload__text">
              Drop file here or <em>click to upload</em> trademark Img
            </div>
            <template #tip>
...
import { getTradeMarkInfo, saveOrUploadTradeMarkInfo, delTradeMark } from '@/api/product/trademark'
import {
...
let trademarkInfoParams = reactive<TradeMarkDataType>({ id: undefined, tmName: '', logoUrl: '' })
...
const addTrademark = () => {
  ...
  trademarkInfoParams.logoUrl = !trademarkInfoParams.logoUrl ? 'http://q1.qlogo.cn/g?b=qq&nk=21171326&s=640' : ''
  //规则清除
...
const confirm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      ...
      if (res.code === 200) {
        dialogTableVisible.value = false
        ElMessage.success(trademarkInfoParams.id ? '更新商品信息成功' : '添加商品成功')
        //进行数据请求，改变Pagination的数据变化
        getHasTradeMarkInfo()
        //收集数据清空
      } else {
        ElMessage.warning(trademarkInfoParams.id ? '更新商品信息失败' : '添加商品失败')
      }
    } else {
      console.log(fields);
...
//上传成功
const handleUploadFileSuccess: UploadProps['onSuccess'] = (response, uploadFile) => {
  //将服务器返回数据收集
